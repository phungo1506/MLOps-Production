...
from . import save
from torch.quantization import QuantStub, DeQuantStub, quantize, default_qconfig, prepare, prepare_qat, convert
...
def train_step(model: torch.nn.Module, 
    ...
    torch.save(quantized_model.state_dict(), model_names)
...
def train(model: torch.nn.Module, 
    ...
    target_dir_path = Path(work_dir)
    target_dir_path.mkdir(parents=True,
                        exist_ok=True)
    model_save_path = str(target_dir_path) + '/' + architecture 
    number = 0
    # Loop through training and testing steps for a number of epochs
    for epoch in tqdm(range(epochs)):
        model_name = model_save_path + str(number) '_.pth'
